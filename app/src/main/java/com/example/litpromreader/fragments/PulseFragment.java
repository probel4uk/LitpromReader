...
import com.example.litpromreader.adapters.PulseAdapter;
...
import com.example.litpromreader.parser.ParsePulse;
...
public class PulseFragment extends Fragment{
    ...
    private ArrayList<Pulse> litpromPulses = new ArrayList<>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ParsePulse parsePulse = new ParsePulse();
        parsePulse.execute();

        try {
            litpromPulses=parsePulse.get();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if(litpromPulses != null && litpromPulses.size() > 0){
            ...
            int textSize = TextSizeConstant.defTextSize;
            if (mSettings.contains(APP_PREFERENCES_TEXT_SIZE)){
                textSize =mSettings.getInt(APP_PREFERENCES_TEXT_SIZE, 0);
            }

            PulseAdapter pulseAdapter = new PulseAdapter(getActivity(), litpromPulses, textSize);

            listView.setAdapter(pulseAdapter);
